...
public class HomePage extends JPanel {
    ...
    public HomePage() {
        ...
        cards.add(buildSummaryCard("Total Saldo", totalSaldoLabel, new WalletIcon(), color(37, 99, 235),
                color(239, 246, 255), null));
        cards.add(buildSummaryCard("Pemasukan Bulan Ini", incomeLabel, new ArrowUpCircleIcon(), color(5, 150, 105),
                color(236, 253, 245), null));
        cards.add(buildSummaryCard("Pengeluaran Bulan Ini", expenseLabel, new ArrowDownCircleIcon(), color(220, 38, 38),
                color(254, 242, 242), null));
        cards.add(buildSummaryCard("Selisih (Profit/Loss)", netLabel, new TrendingUpIcon(), color(5, 150, 105),
                color(236, 253, 245), netSubtitle));
        root.add(cards);
        ...
        // Load all data from backend on init
    ...
    private JComponent buildSummaryCard(String title, JLabel valueLabel, Icon icon, Color accent, Color accentBg,
    ...
    private void refreshData() {
        ...
        double change = netLastMonth == 0 ? 0
                : ((double) (netThisMonth - netLastMonth) / Math.max(1, Math.abs(netLastMonth))) * 100;
        ...
        for (int i = 0; i < months.size(); i++) {
            ...
            incomes[i] = (int) transactionSnapshot.transactions().stream()
                    .filter(t -> t.yearMonth().equals(ym) && t.isIncome())
                    .mapToLong(TransactionStore.Transaction::amount).sum();
            expenses[i] = (int) transactionSnapshot.transactions().stream()
                    .filter(t -> t.yearMonth().equals(ym) && !t.isIncome())
                    .mapToLong(TransactionStore.Transaction::amount).sum();
        }
        ...
        Color[] palette = new Color[] { color(37, 99, 235), color(5, 150, 105), color(220, 38, 38), color(245, 158, 11),
                color(100, 116, 139), color(14, 165, 233) };
        Map<String, Long> groupedExpenses = transactionSnapshot.transactions().stream()
                .filter(t -> t.yearMonth().equals(now) && !t.isIncome())
                .collect(java.util.stream.Collectors.groupingBy(TransactionStore.Transaction::category,
                        java.util.stream.Collectors.summingLong(TransactionStore.Transaction::amount)));
        int idx = 0;
    ...
    private void loadAllDataFromBackend() {
        showLoading(true);
    ...
    private JComponent billItem(String name, String amount, String due) {
        RoundPanel p = new RoundPanel(10, new Color(255, 251, 235), new Color(253, 230, 138)); // amber-50 +
                                                                                               // border-amber-200
    ...
    static class TrendChartPanel extends JPanel {
        ...
        private void drawSeries(Graphics2D g2, int[] data, Color col, int padL, int padR, int padT, int padB, int w,
                int h, int max) {
            if (data.length == 0)
                return;
            g2.setColor(col);
        ...
        private void drawLegend(Graphics2D g2, int x, int y, Color col, String name) {
            g2.setColor(col);
            g2.fillRect(x, y - 8, 14, 3);
            g2.setColor(new Color(30, 41, 59));
            g2.drawString(name, x + 18, y);
        }
    }
    ...
    static class CategoryPieChartPanel extends JPanel {
        static class Slice {
            ...
            Slice(String n, double v, Color c) {
                name = n;
                value = v;
                color = c;
            }
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (Slice s : slices) {
                ...
                g2.setColor(new Color(30, 41, 59));
                g2.drawString(s.name, lx + 18, ly + 11);
                ly += 18;
            }
            g2.dispose();
        }
